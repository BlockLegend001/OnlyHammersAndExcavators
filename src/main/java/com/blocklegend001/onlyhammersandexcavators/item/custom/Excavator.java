...
public class Excavator extends ShovelItem {
    ...
    public boolean postMine(ItemStack stack, World world, BlockState state, BlockPos pos, LivingEntity miner) {
        ...
        BlockPos.iterateOutwards(pos, offset.getX(), offset.getY(), offset.getZ()).forEach(e -> {
            ...
            int excavatorMiningLevel = this.getMaterial().getMiningLevel();
            if (isSuitableFor(targetBlockState) && targetBlockState.isIn(BlockTags.SHOVEL_MINEABLE) && excavatorMiningLevel >= requiredToolLevel) {
